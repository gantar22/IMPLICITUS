...
public class LevelLoader : ScriptableObject {
	...
	private void rerandomizeLevelRandom() {
		...
		int numVariables = numCombinatorsToApply + Random.Range(-1, 3); // How many variables will be used.
		...
		int evaluationLimit = 20; // Limit on the number of times a term can be evaluated, to detect (likely) infinite loops.

		// ---- Term Generation ----
		// Choose the random spells and start constructing the Term leafList
		List<Spell> spellsToApply = new List<Spell>();
		List<Term> leafList = new List<Term>();
		for (int i=0; i < numCombinatorsToApply; i++) {
			...
			leafList.Add(Term.Leaf(Sum<Combinator, Lambda.Variable>.Inl(randSpell.combinator)));
		...
		for (int i = 0; i < leafList.Count - 1; i++) {
			int toSwap = Random.Range(i, leafList.Count);
			Term tempLeaf = leafList[toSwap];
			leafList[toSwap] = leafList[i];
			leafList[i] = tempLeaf;
		}
		...
		while (elimRules.Count > 0 && evaluationCount < evaluationLimit) {
			evaluationCount++;
		...
		if (evaluationCount >= evaluationLimit || containsCombinator(term)) {
			rerandomizeLevelRandom();
			return;
		}
